...
---- Archetype
Create table archetype /* Create a table with two columns. one for id and the other for the distinct value. */
( archetype_id Serial primary key,
  gun_archetype varchar(100)
 );

Insert into archetype (gun_archetype) select distinct gun_archetype from guns; /* Insert the unique values into archetype */

alter table guns /* Add the new id column to the original guns table */
add column archetype_id int; 

update guns /* populate the new column with the corresponding archetype_id number from the archetype table */
set archetype_id = archetype.archetype_id from archetype
where guns.gun_archetype = archetype.gun_archetype;

alter table guns /* set foreign key constraint */
add foreign key (archetype_id) references archetype (archetype_id);

alter table guns /* drop the column with the full values so that only the new column with the archetype_id exists */
drop gun_archetype;

/* Repeat for remaining non-integer or primary key columns */
---- Source
Create table source
...
/* view (Why? views are basically virtual tables that take up no space on the database. 
We can use it to view our guns table with full values instead of numbers, thus making it easier to query and read. */
drop view if exists v_guns;
create view v_guns as 
SELECT stg.gun_id
     , stg.gun_name
	 , stg.gun_rof
	 , a.gun_archetype
	 ...
	 , e.gun_type
from guns stg
Inner join archetype a on stg.archetype_id = a.archetype_id
Inner join element b on stg.element_id = b.element_id
Inner join rarity c on stg.rarity_id = c.rarity_id
Inner join source d on stg.source_id = d.source_id
Inner join type e on stg.type_id = e.type_id;
